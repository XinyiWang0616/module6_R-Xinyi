...
**Answer Q.1**
It refers to the lack of confidence in predcitions due to the incomplete knowledge, data limitations, and natural variability in ecological systems.
...
**Answer Q.2**Knowing forecast uncertainty would help managers make decisions under consideration of risk. If drinking water managers received a toxic algal bloom forecast with low uncertainty, they probably need to act immediately to protect public health. However, when they recieved a forecast with high uncertainty, they may increase sampling and improve monitoring first rather than immediate interventions
...
**Answer Q.4** Increasing water temperature can reduce dissolved oxygen levels and promote harmful algal blooms that degrade water quality and threaten the ecosystem.
...
**Answer Q.5** Yes, the figure suggests a strong positive relationship between air temperature and water temperature. Both variables show similar seasonal patterns, indicating that changes in air temperature would strongly influence water temperature.
...
**Answer Q.6**The multiple gray lines represent ensemble forecasts of future air temperature. Specifically, each line is one individual forecast output generated from the forecast system, and together they show the range of possible outputs and the forecasting uncertainty.
...
**Answer Q.7**The multiplt linear regression model predicts today's water temperature using today's air temperature and yesterday's water temperature. Today's air temperature represents current atmospheric condition, while yesterday's water temperature represents inherent thermal condition of the lake. 
...
**Answer Q.8**Coefficients suggest past water temperature is the dominant drive of today's water temperature in the model, while air temperature has a relatively smaller but still positive influence. 
The standard errors for wtemp_yday and airt are small, suggesting we are quite confident that these coefficients are well estimated. 
...
**Answer Q.9** The model performs very well, which produces high r2(0.9) and relatively small prediction errors(0.61).
...
water_temperature <- rep(NA, 11) # change code here
...
**Answer Q.11** Yes, the forecasted water temperature generally captures the dynamics of forecasted air temperature because the water temperature model explicitly uses forecasted air temperature as an input.  
...
**Answer Q.12**Forecast uncertainty increases farther into the future, as shown by the widening spread of forecast lines.
...
**Answer Q.13** The deterministic forecast only has a single predicted temperature output, showing a warming and then cooling pattern. However, the probabilistic forecast shows a range of possible outputs with increasing uncertainty over time. While water temperature are likely to peak midweek and then decline, the exact temperature later in the week are still uncertain and can vary substantially.
...
**Answer Q.14** Parameter uncertainty could be reduced by collecting more high-quality data across diverse conditions, improving predictor set to better capture key process dynamics, and using data assimilation methods to better constrain parameter estimates.
...
**Answer Q.15** It would increase when the starting state of the system is poorly known, such as when measurements contain errors, observations are missing, or rapid environmental changes occur before the forecast begins.
...
**Answer Q.16** Parameter uncertainty contributes the most to total forecast uncertainty, as shown by the largest portion of variance in the figure above.
...
**Answer Q.17** 
Yes, this new model would likely have different quantities of uncertainty.  

*Driver uncertainty*
Driver uncertainty would likely increase because forecasts of solar radiation and precipitation introduce additional uncertain inputs.
*Parameter uncertainty*
Parameter uncertainty might decrease if the additional variables improve model performance, and it may also increase when data is limited.
*Initial conditions uncertainty*
Initial conditions uncertainty would likely remain, which depends on hoe well the starting state of water temperature is known.
*Process uncertainty*
Process uncertainty would likely decrease because the model better represents physical drivers of water temperature. 
...
**Answer Q.19** The new multiple linear regression model predicts today's water temperature using yesterday's water temperature, today's air temperature, solar radiation, and wind speed. Compared to the previous model, which used only yesterday's water temperature and air temperature, this model incorporates more physical drivers.
...
for(d in 2:length(forecasted_dates)) {
  ...
  #run model - REVISE THIS!!
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] + temp_driv$shortwave_radiation * coeffs[4] + temp_driv$wind_speed * coeffs[5] + rnorm(n = n_members, mean = 0, sd = sigma)
...
**Answer Q.25** Parameter uncertainty contributes most to total forecast uncertainty, as shown by the largest portion of variance in the figure above.
...
Parameter uncertainty contributes most to the forecast uncertainty in both the two models. Also both models show uncertainty increasing into the future.

Compare the contribution of driver uncertainty:     
Driver uncertainty tends to be larger in the multiple linear regression model because it has more drivers compared to the previous model.

Compare the contribution of parameter uncertainty:  
Parameter uncertainty is the largest contributor in both models, and it could be larger in the multiple linear regression model because there are more coefficients. It also grows over time because daily forecast depends on previous predicted values, which means uncertainty in coefficients would compound forward.

Compare the contribution of process uncertainty:  
Process uncertainty is not the dominant contributor of forecast uncertainty and its contribution to uncertainty is roughly similar in both models. This could be because the additional predictors are not strongly informative to explain the variability in water temperature, or because both models are linear regression models iwth little change in model structure.

Compare the contribution of initial conditions uncertainty:     
Initial conditions uncertainty stays about the same across models, suggesting adding more predictors does not directly improve the uncertainty of the initial water temperature measurements.
...
**Answer Q.27**
I would spend most of money on improving the quantity and quality of the training dataset because the parameter uncertainty is the largest contributor to total forecast uncertainty. I would also invest in improving driver data by installing on-site weather monitoring systems in order to obtain more accurate measurements of climatic drivers. Finally, I would add an water temperature sensor and calibrate sensors to reduce process and initial conditions uncertainty by better capturing weather dynamics and ensuring reliable starting conditions.
